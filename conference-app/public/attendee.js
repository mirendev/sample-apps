...
document.addEventListener('DOMContentLoaded', function() {
            ...
            try {
                ...
                if (response.ok) {
                    if (isRegistered) {
                        ...
                        document.querySelector(`.talk-card[data-talk-id="${talkId}"]`).classList.remove('registered');
                        
                        // Sad message
                        const sadMessages = ["😢 We'll miss you!", "💔 Breaking up is hard", "😔 Maybe next time?"];
                        ...
                        document.querySelector(`.talk-card[data-talk-id="${talkId}"]`).classList.add('registered');
                        
                        // Happy message and mini confetti
                        const happyMessages = ["🎉 Great choice!", "✨ You're gonna love it!", "🚀 See you there!"];
                        ...
                        for (let i = 0; i < 20; i++) {
                            setTimeout(() => {
                                ...
                                confetti.style.backgroundColor = ['#f44336', '#4caf50', '#2196f3', '#ffeb3b'][Math.floor(Math.random() * 4)];
    ...
    async function loadRegisteredTalks() {
        ...
        try {
            ...
            if (talks.length > 0) {
                registeredTalksList.innerHTML = talks.map(talk => `
                    <div class="talk-item">
                        <h3>${talk.title}</h3>
